...
def mean_objective_deficiency(instance_results: npt.ArrayLike, bench_min: npt.ArrayLike,
    ...
    return np.mean(normalize(zs, bench_min[-1], val_range)).round(10)
...
def run_PPAs(benchmark: Benchmark, loops: int = 100, threads: int = 10):
    inp = ((np.random.randint(10_000), benchmark) for _ in range(loops))
...
def get_true_results(bench: Benchmark, loops: int = 100, threads: int = 10):

    instance_results = run_PPAs(bench, loops, threads)

    val_range = bench.get_true_range()
    bench_min, bench_max = bench.get_true_extremes()

    mbf = mean_objective_deficiency(
        instance_results, bench_min, val_range, [], bench.max_dis)
